...
router.get('/:id', (req, res) => {
    ...
    taskModel.findById(taskId).then(taskFound => {
        if (!taskFound) {
...
router.post('/', (req, res) => {
    ...
    newTask.save()
      .then(savedTask => {
        res.location(`${req.protocol}://${req.get('host')}${req.originalUrl}/${savedTask._id}`).json(savedTask);
      })
      .catch(error => {
        res.status(500).json({ error: 'Internal Server Error' });
      });
...
router.put('/:id', (req, res) => {
    ...
    taskModel.findById(taskId)
      .then(task => {
        if (!task) {
            return res.status(404).json({ msg: `A task with id=${taskId} was not found` });
        }
